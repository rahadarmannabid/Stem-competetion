...
figure1=figure;
t1=toc;
x=[50 60 40 20];     % initial condition (#cars at first time)
xi=x;
...
b=[0 1]; % an auxiliary matrix for determine which light must be green
greentime=[0 5 5 2 6 8 1 5 5 5];
...
exitcar=20;              % #cars that exit in each step from green light
...
figure
cost=1;
plot(time,xstore(:,1)*cost,'-ro',time,xstore(:,2)*cost,'-go',time,xstore(:,3)*cost,'-bo',time,xstore(:,4)*cost,'-ko',time,(sum(xstore'))','-r*')
legend('sreet1','sreet2','sreet3','sreet4','Total')